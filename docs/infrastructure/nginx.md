...
Replace `yourdomain.com` in the template before enabling the site. Admin subdomain: `admin.yourdomain.com` — base path `/` (not `/admin/`). Nginx proxies `/api/` to the API and serves `/assets/` from `apps/admin/dist/assets/`.
